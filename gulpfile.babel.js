...
function watch() {
  ...
  gulp.watch('src/assets/scss/**/*.scss').on('all', gulp.series( sass, rtl ) );
...
gulp.task('build',
 gulp.series(clean, sass, rtl, 'webpack:build', tinymce, images, copy));
...
function version() {
    ...
    return gulp.src([
        ...
        // Plugin header
        .pipe($.replace(/(private \$version = ').*?(')/, "$1" + pkg.version + "$2"))
        // readme.txt
...
function copyVendorLicenses() {
  ...
  return gulp.src( PATHS.licenses, { allowEmpty: true, base: '*' } )
    .pipe( $.rename( function( file ) {
      file.dirname = file.dirname.replace( /.*\\(.*)$/, '\$1' );
      file.extname = '.md';
      return file;
    } ) )
